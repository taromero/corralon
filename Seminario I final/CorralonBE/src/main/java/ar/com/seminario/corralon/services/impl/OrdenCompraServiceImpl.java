...
public class OrdenCompraServiceImpl implements OrdenCompraService {
	...
	public void generarOrdenCompra(Long idUsuario, String razonSocialProveedor,
			List<String> descripciones, List<String> cantidades,
		...
		for (int i = 0; i < descripciones.size(); i++) {
			...
			cant = Integer.parseInt(cantidades.get(i));
			MaterialProveedor material = materialService
					.findMatProvByDescripcion(descripcion);
		...
		for(Madera m : maderas){
			ItemOrdenCompra item = new ItemOrdenCompra();
			MaterialProveedor material = materialService
				.findMatProvByDescripcion(m.getDescripcion());
			item.setMaterial(material);
			item.setCantidad(1);
			item.setOrdenCompra(OCActual);
			precioTotal += item.getMaterial().getPrecio()
	...
	@Override
	public void merge(OrdenCompra entity) {
		ordenCompraDAO.merge(entity);
	}
