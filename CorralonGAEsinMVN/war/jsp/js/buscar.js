		...
		$.ajax({
			...
			success: function(data) {
				...
				for (var index in data.itemsPresupuesto) {
					...
					var unidadMaterial = data.itemsPresupuesto[index].material.unidad;
					if(unidadMaterial){
						l1 = '<td>' + cortarNombreMaterialParaLista(bDesc) + '</td>';
						lUnidad = '<tr><td>' + unidadMaterial + '</td>';
						if(esMadera(bDesc)){
							var bLargo = data.itemsPresupuesto[index].material.largo;
							l1 = '<td>' + cortarNombreMaterialParaLista(bDesc) + '</td>';
							l2 = '<td>' + '1'+ '</td>';
	            ...
	            $('#descuentoPBuscar').html(data.cliente.descuento + '%');
	            $('#totalCdPBuscar').html((data.total - data.total*data.cliente.descuento/100).toFixed(2));

	            if(correrFilas3){
					for(var i = 0; i<cantMat3; i++){
						if(!esMadera(data.itemsPresupuesto[i].material.descripcion)){
							$('#item3' + i).prepend('<td></td>');
						}
					}
					correrFilas3 = false;
				}
